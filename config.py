...
parse.add_argument("-p", "--proto", choices=["near","rep","cen", "cos"], default="rep",
...
parse.add_argument("-d", "--data", choices=["german", "adult"], default="adult", help="Choosing a dataset.")
...
global model
if args.data == "german":
    dump(v_model,"German/v_model.joblib")
    model = Net("German/v_model.joblib")
    pass
elif args.data == "adult":
    dump(v_model, "Adult/v_model.joblib")
    model = Net("Adult/v_model.joblib")
    pass
...
CEs = pd.DataFrame(creator.createCFs(args.proto, dataset=args.data, device=args.gpu, func = args.func), columns=data.columns.values)            # 默认german,cos,fcs
